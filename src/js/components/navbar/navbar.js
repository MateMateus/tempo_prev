...
function initNavbarEvents() {
    ...
    const closeSearchMobile = () => {
        const centerNav = document.getElementById('navbar-search-center');
        if (centerNav) {
            centerNav.classList.remove('bem-navbar__center--mobile-active');
        }
        if (searchInput) {
            searchInput.value = '';
            searchInput.blur();
                ...
                try {
                    ...
                    if (geoData && geoData.results && geoData.results.length > 0) {
                        ...
                        autocompleteDropdown.innerHTML = sugestoes.map(item => {
                            const estadoFormatado = (item.admin1 && item.admin1.toLowerCase() !== item.name.toLowerCase()) 
                                ? item.admin1 
                                : (item.country_code ? item.country_code.toUpperCase() : 'BR');
                            const nomeComp = `${item.name}, ${estadoFormatado}`;
                            return `
                                <div class="bem-navbar__autocomplete-item" data-cidade="${nomeComp}">
                                    <div>
                                        <strong>${item.name}</strong>
                                        <div class="bem-navbar__autocomplete-sub">${estadoFormatado} • Brasil</div>
                                    </div>
                        ...
                        autocompleteDropdown.querySelectorAll('.bem-navbar__autocomplete-item').forEach(item => {
                            item.addEventListener('click', () => {
                                ...
                                window.location.hash = `#clima/${gerarSlugCidade(cidadeSel)}`;
                                closeSearchMobile();
                            });
                        });
                    } else {
    ...
    if (searchForm) {
        searchForm.addEventListener('submit', (e) => {
            ...
            if (cidade) {
                window.location.hash = `#clima/${gerarSlugCidade(cidade)}`;
                closeSearchMobile();
            }
        });
    }
